...
import { z } from "zod";
import { loadCecoScope, syncCecoScope } from "@/lib/ceco";
import { createSupabaseServerClient, getCurrentUser } from "@/lib/db/server";

export type CecoActionResult = { ok: boolean; error?: string; pages?: number; version?: string };

/** Pull a fresh copy of CECO's scope now, instead of waiting for the next tick. */
export async function syncCecoAction(): Promise<CecoActionResult> {
  ...
  try {
    ...
    return { ok: true, pages: scope.pages.length, version: scope.app.version };
  } catch (e) {
